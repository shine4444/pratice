...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagTemperature)
    {
        info->event[info->num] = EVENT_TEMPERATURE;
        info->num++;
        dataPoints->valueTemperature = gizX2Y(TEMPERATURE_RATIO,  TEMPERATURE_ADDITION, issuedData->attrVals.valueTemperature); 
    }



    if(0x01 == issuedData->attrFlags.flaggear)
    {
        info->event[info->num] = EVENT_GEAR;
        info->num++;
        dataPoints->valuegear = gizX2Y(GEAR_RATIO,  GEAR_ADDITION, issuedData->attrVals.valuegear); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuegear != cur->valuegear)
    {
        GIZWITS_LOG("valuegear Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valuegear = gizY2X(GEAR_RATIO,  GEAR_ADDITION, dataPoints->valuegear); 
